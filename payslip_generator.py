...
# # df.to_excel("processed_employees.xlsx.xlsx", index=False)
...
# # # Function to generate a payslip PDF
...
# #     pdf.set_font("Arial", '', 12)
# #     pdf.cell(0, 10, f"Basic Salary: KES {row['BASIC SALARY']:.2f}", ln=True)
# #     pdf.cell(0, 10, f"Allowances: KES {row['ALLOWANCES']:.2f}", ln=True)
# #     pdf.cell(0, 10, f"Deductions: KES {row['DEDUCTIONS']:.2f}", ln=True)
# #     pdf.ln(5)

# #     # Net Salary
# #     pdf.set_font("Arial", 'B', 12)
# #     pdf.cell(0, 10, f"Net Salary: KES {row['NET SALARY']:.2f}", ln=True)
...
# # Emailing each payslip via smtplib
# def send_email(to_email, subject, body, attachment):
#     from_email = "lindsaychimanga198@gmail.com"  # Your email address
#     password = "komr xwmd wnds rhqk"  # Your email password (or app password if 2FA is enabled)
...
#     try:
#         # Connect to Gmail's SMTP server (adjust the server and port if using another email provider)
...
# # Loop through the DataFrame and send emails with payslips
# for _, row in df.iterrows():
#     to_email = row['EMAIL']
#     name = row['NAME']
